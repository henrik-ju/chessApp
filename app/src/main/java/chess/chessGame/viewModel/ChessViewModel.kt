...
class ChessViewModel(
    ...
    private val _playersJoined = mutableIntStateOf(0)
    val playersJoined: State<Int> get() = _playersJoined
    ...
    private val _opponentUsername = MutableStateFlow<String?>(null)
    val opponentUsername: StateFlow<String?> get() = _opponentUsername
    private val _opponentPhotoUrl = MutableStateFlow<String?>(null)
    val opponentPhotoUrl: StateFlow<String?> get() = _opponentPhotoUrl

    init {
        _game.value.onPromotion = { _, position -> _promotionPosition.value = position }
        viewModelScope.launch {
            try {
                ...
                val (username, photoUrl) = firebaseService.getOpponentInfo(gameId, currentUserId)
                _opponentUsername.value = username
                _opponentPhotoUrl.value = photoUrl
        ...
        gameListenerRegistration = firebaseService.listenToGame(gameId) { lobby ->
            _playersJoined.intValue = lobby.playerCount
            lobby.fen?.let { fen ->
                if (fen.isNotEmpty() && fen != _game.value.toFen()) {
                    val newGame = ChessGame.fromFen(fen)
                    newGame.onPromotion = { _, position -> _promotionPosition.value = position }
                    _game.value = newGame
                    ...
                    if (success) {
                        ...
                        withContext(Dispatchers.Main) {
                            if (transactionSuccess) {
                                ...
                                _moveError.value = null
                                if (currentGame.isFinished) {
                                    _gameResult.value = determineGameResult(currentGame)
                                }
                            } else {
            ...
            withContext(Dispatchers.Main) {
                if (!transactionSuccess) {
                    ...
                    if (gameInstance.isFinished) {
                        _gameResult.value = determineGameResult(gameInstance)
                    }
                }
            }
        }
    ...
    fun processEncryptedMessages(encryptedList: List<ChatMessage>) {
        val secretKey = gameSessionSecretKey ?: return
    ...
    fun sendChatMessage(plaintext: String) {
        ...
        if (secretKey == null || plaintext.isBlank()) return
